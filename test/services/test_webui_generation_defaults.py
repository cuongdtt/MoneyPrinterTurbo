...
def test_fal_source_shows_unchecked_paid_task_confirmation():
    test_app_config = dict(config.app, video_source="fal", fal_api_key="")
    test_ui_config = dict(config.ui, language="en", voice_mode="none")
    ...
    assert app.session_state["video_source_select_en"] == "fal"
    assert _widget_by_key(app.checkbox, "fal_confirm_charge").value is False
    durations = _widget_by_key(app.selectbox, "video_clip_duration_select").options
    assert min(map(int, durations)) == 3
    assert max(map(int, durations)) == 15
